  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        :key="userAddress.id"
        @click="changeDefaultAddress(userAddress)"
      >
        <span
          class="username "
          :class="{ selected: userAddress.isDefault === '1' ? true : false }"
          >{{ userAddress.consignee }}</span
        >
        <p>
          <span class="s1">{{ userAddress.userAddress }}</span>
          <span class="s2">{{ userAddress.phoneNum }}</span>
          <span class="s3" v-show="userAddress.isDefault === '1' ? true : false"
            >默认地址</span
          >
        </p>
      </div>
      ...
      <div class="detail">
        ...
        >
          <li>
            <img :src="detail.imgUrl" alt="" width="80" height="80" />
          </li>
          <li>
            <p>
              {{ detail.skuName }}
...
export default {
  ...
  methods: {
    ...
    //修改默认地址
    changeDefaultAddress(tradeAddress) {
      this.userAddressList.forEach((item) => {
        item.isDefault = "0";
      });
      tradeAddress.isDefault = "1";
    },
    ...
    async submitTrade() {
      ...
      try {
        ...
        const orderComment = this.messages;
        const {
          consignee,
          userAddress: deliveryAddress,
          phoneNum: consigneeTel,
        } = this.defaultAddress;
        const orderDetailList = this.detailArrayList;
        const tradeInfo = {
          orderComment,
          consignee,
          deliveryAddress,
        ...
        const orderId = await this.$store.dispatch("getSubmitTrade", {
          tradeNo,
          tradeInfo,
        });
        //订单创建
        alert("订单创建成功，自动去往支付界面");
